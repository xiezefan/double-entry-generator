...
"$ROOT_DIR/bin/double-entry-generator" translate \
    --provider cmb_credit \
    --config "$ROOT_DIR/example/cmb_credit/config.yaml" \
    --output "$ROOT_DIR/test/output/test-cmb-credit-output.beancount" \
    "$ROOT_DIR/example/cmb_credit/cmb_credit.json"
...
if [ $? -ne 0 ]; then
    echo "[FAIL] CmbCredit provider output is different from expected output."
    exit 1
fi

echo "[PASS] All CmbCredit provider tests!"
